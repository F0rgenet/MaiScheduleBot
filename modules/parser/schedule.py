...
from .datatypes import Week, Day, Subject
from accessify import private


class ScheduleParser(object):
    def __init__(self):
        self.__url = "https://mai.ru/education/studies/schedule/index.php"
    ...
    @private
    def parse_day(self, day_soup: PageElement | BeautifulSoup) -> Day | None:
        subjects = []
    ...
    def parse_schedule(self, group: str, week: int) -> Week:
        user_agent = UserAgent().chrome
        link = f"{self.__url}?group={group}&week={week}"
        ...
        for day in days_soup.findChildren(recursive=False):
            if day:
                days.append(self.parse_day(day))
        return Week(days)